...
class _TaskListSectionState extends State<TaskListSection> {
  ...
  Color _accentColor(Task task) {
    ...
    final diff = _calendarDayDifference(task.dueDate);
                    ...
                    children: [
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          onSubmitted: (text) =>
                              _addSubtask(context, task, text),
                        ),
                      ),
                      const SizedBox(width: 8),
                      GestureDetector(
                        onTap: () {
                          _addSubtask(context, task, _controller(task.id).text);
                        },
                        child: Container(
                          width: 34,
                          height: 34,
  ...
  Future<void> _addSubtask(
    ...
    if (t.isNotEmpty) {
      try {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not add the step. Try again.')),
        );
      }
    }
  }
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Could not delete the task. Try again.'),
                  ),
                );
              }
            },
  ...
  void _showEditSubtaskDialog(
      BuildContext context, int taskId, int subtaskId, String current) {
            ...
            onPressed: () async {
              try {
                await Provider.of<TaskState>(
                  context,
                  listen: false,
                ).editSubtask(taskId, subtaskId, c.text);
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Could not save the step. Try again.'),
                  ),
                );
              }
            },
  ...
  void _showEditDialog(BuildContext context, Task task) {
    ...
    DateTime selected = task.dueDate;
            ...
            TextButton(
              onPressed: () async {
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Could not save the task. Try again.'),
                    ),
                  );
                }
              },
